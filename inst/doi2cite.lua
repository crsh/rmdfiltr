...
function Meta(m)
    ...
    for bibfile, _ in pairs(bibpaths) do
        ...
        if f then
            ...
            if entries_str then
                -- Extract entries and keys from the current file
                local new_doi_entry_map = get_doi_entry_map(entries_str)
                local new_doi_key_map = get_doi_key_map(entries_str)
                
                for doi, entry in pairs(new_doi_entry_map) do
                    if not doi_entry_map[doi] then
                        -- Only add entry if DOI is not already present
                        doi_entry_map[doi] = entry
                        local key = new_doi_key_map[doi]
                        key_list[key] = true
                        doi_key_map[doi] = key
                    else
            ...
            -- Handle case where the file doesn't exist or cannot be opened
...
function get_entrydoi(entry_string)
    local doi = entry_string:match('[Dd][Oo][Ii]%s*=%s*["{]*(.-)["}],?') or ''
    return doi
end
...
function get_doi_entry_map(bibtex_string)
    local entries = {};
    for entry_str in bibtex_string:gmatch('@.-%s\n*}\n') do
